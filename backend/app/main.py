...
def generate_grid(min_lat, min_lon, max_lat, max_lon, step=3.0):
    ...
    if len(lats) == 0:
        lats = np.array([min_lat])
    if len(lons) == 0:
        lons = np.array([min_lon])
    lat_grid, lon_grid = np.meshgrid(lats, lons)
    return lat_grid.flatten(), lon_grid.flatten()


def get_cache_key(
    prefix: str, min_lat: float, min_lon: float, max_lat: float, max_lon: float
) -> str:
...
async def get_vessels(
    ...
    if cached:
        print(
            f"[VESSELS] Cache hit for bbox: {min_lat:.1f},{min_lon:.1f} ({len(cached.get('features', []))} vessels)"
        )
        return cached
    ...
    print(
        f"[VESSELS] Cached {len(filtered)} vessels for bbox (Total tracked: {len(vessels_cache)})"
    )
...
async def get_waves(
    ...
    try:
        ...
        for i, response in enumerate(responses):
            ...
            swell_height = (
                round(swell_height, 2) if swell_height and not np.isnan(swell_height) else 0
            )
...
async def get_wave_point(lat: float = Query(...), lon: float = Query(...)):
